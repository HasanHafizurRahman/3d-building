...
export default function Home() {
  ...
  return (
    <main className="min-h-screen w-full bg-white text-black">
      ...
      {/* Hero Section with Building Selector */}
      <section className="pt-32 pb-12 px-6 sm:px-12 relative overflow-hidden">
        <div className="max-w-[1440px] mx-auto">
          {/* Animated Gradient Orbs */}
          <div className="absolute top-0 left-1/4 w-96 h-96 bg-black/5 rounded-full blur-3xl animate-pulse" />
          <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-black/5 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />

          <div className="relative text-center mb-12">
            <div className="inline-flex items-center gap-2 bg-black/5 backdrop-blur-xl border border-black/10 rounded-full px-6 py-2 mb-6">
              <Sparkles className="w-4 h-4 text-black" />
              <span className="text-sm font-medium text-gray-700">Discover Luxury Living</span>
            </div>
            ...
            <p className="text-xl text-gray-600 max-w-2xl mx-auto mb-12 leading-relaxed">
              Experience architectural excellence with our curated collection of premium real estate.
              Where sophistication meets innovation.
            </p>

            {/* Enhanced Building Selector */}
            <div className="flex flex-wrap justify-center gap-4">
              {buildingsData.map((building) => (
                <button
                  key={building.id}
                  onClick={() => setSelectedBuildingId(building.id)}
                  className={`group relative overflow-hidden px-8 py-4 rounded-2xl font-bold transition-all duration-300 ${selectedBuildingId === building.id
                      ? 'bg-black text-white shadow-[0_0_40px_rgba(0,0,0,0.3)]'
                      : 'bg-black/5 backdrop-blur-xl text-black border border-black/10 hover:bg-black/10 hover:border-black/20'
                    }`}
                >
                  ...
                  {selectedBuildingId === building.id && (
                    <div className="absolute inset-0 bg-gradient-to-r from-gray-900 to-black opacity-20 animate-pulse" />
                  )}
                </button>
              ))}
            </div>
